...
function row_diff(source_data in dbms_tf.table_t, target_data in dbms_tf.table_t, id_cols in dbms_tf.columns_t
...
function row_col_diff(source_data in dbms_tf.table_t, target_data in dbms_tf.table_t, id_cols in dbms_tf.columns_t
    ) return varchar2 sql_macro(table)
is
    l_sql varchar2(4000);
    l_id_cols varchar2(4000);
...
select * from row_col_diff(source, target, columns(id,code))
/
