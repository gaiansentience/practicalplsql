--create.procedure.compare_binary_search.menu_vectors.sql

prompt create a procedure to test different model searches between float32 and binary

create or replace procedure compare_binary_search_menu_vectors(
    p_search_term in varchar2, p_model in varchar2
--menu_vectors embeddings should be created with mx_bai_embed_large_v1 
...
from 
    menu_vectors v
    join menu_items i on i.item_id = v.item_id
order by 
    vector_distance(
        v.embedding
...
order by 
    vector_distance(
        v.embedding_binary
        , to_binary_vector(vector_embedding(MXBAI_EMBED_XSMALL_V1 using 'after school snack' as data))
        , jaccard)
fetch first 5 rows only
/

select embedding1_model from menu_vectors;
...
order by 
    vector_distance(
        v.embedding1
        , vector_embedding(MXBAI_EMBED_LARGE_V1 using 'after school snack' as data)
        , cosine)
fetch first 5 rows only
/

select embedding1 from menu_vectors;
