...
prompt populating collection with empty cursor: cursor iteration control as array constructor
declare
...
begin
    ...
    l_rows := null;
    l_rows := t_rows(for r in values of c(3) sequence => r);
    print_collection_state(l_rows);

    dbms_output.put_line('fetch 0 rows');
    l_rows := null;
    l_rows := t_rows(for r in values of c(0) sequence => r);
    print_collection_state(l_rows);
...
/* Script output
populating collection with empty cursor: cursor iteration control as array constructor
fetch 3 rows
