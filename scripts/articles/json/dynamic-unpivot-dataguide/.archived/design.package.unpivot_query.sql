prompt design.package.unpivot_query.sql

--use json_table_expression as CTE
...
as
    ...
    function build_unpivot_expression(
        p_cols in dynamic_json.t_columns,
        p_json_table_expression in varchar2
    ) return varchar2;

    procedure print_unpivot_expression(
        p_jdoc in $if dbms_db_version.version >= 21 $then json $else clob $end ,
        p_array_path in varchar2 default null
    );
...
+';
    ...
    begin
        ...
        l_sql := build_unpivot_expression(l_cols, l_json_table_sql);
        dbms_output.put_line(l_sql);
    end print_unpivot_expression;
...
begin
    ...
    dynamic_json.print_unpivot_expression(l_json, null);
            
    l_json := dynamic_json.get_test_json('nested');
    dynamic_json.print_unpivot_expression(l_json, '.my_shapes');
