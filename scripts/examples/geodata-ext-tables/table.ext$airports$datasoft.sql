prompt public dataset downloaded from https://data.opendatasoft.com
...
--drop table ext$airports$datasoft;
CREATE TABLE ext$airports$datasoft 
...
--           NODISCARDFILE
--           NOLOGFILE
           BADFILE ETL_STAGE_DIR:'datasoft-airports.bad'
           DISCARDFILE ETL_STAGE_DIR:'datasoft-airports.discard'
           LOGFILE ETL_STAGE_DIR:'datasoft-airports.log'
           skip 5 
     ...
     LOCATION ('datasoft-airports.csv')
  )
  REJECT LIMIT UNLIMITED;

--select * from ext$airports$datasoft WHERE ROWNUM <= 5;
